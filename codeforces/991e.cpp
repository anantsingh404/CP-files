...
int solve(string &str1, string &str2, string &c, int i, int j, vector<vector<int>> &dp) {
    if (i == 0 && j == 0)
    {
         return 0;
    }
    ...
    if (i > 0) 
    {    
        cost = solve(str1, str2, c, i - 1, j, dp);
        if(str1[i-1]!=c[idx])
        {
            ++cost;
        }
    ...
    if (j > 0) {
        int f=str2[j-1]!=c[idx];
        cost = min(cost, f+solve(str1, str2, c, i, j - 1, dp));
    }
...
int main() {
    ...
    while (t--) {
        ...
        cout << solve(str1, str2, c, n, m, dp) << endl;
    }
}
