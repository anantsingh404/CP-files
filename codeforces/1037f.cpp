...
struct Edge {
    int u, v;
    ll cost;
    bool contributes; // whether it currently adds to total
};
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            edges.push_back({u, v, c, color[u] != color[v]});
            adj[u].push_back(i);
            adj[v].push_back(i);
        }
        ...
        while (q--) {
            ...
            for (int edgeIdx : adj[v]) {
                ...
                if (e.contributes && color[u] == newColor) {
                    total -= e.cost;
                    e.contributes = false;
                }
                
                else if (!e.contributes && color[u] != newColor) {
                    total += e.cost;
                    e.contributes = true;
                }
            }
