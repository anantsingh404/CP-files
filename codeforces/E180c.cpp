...
//Main Function:
void solve(int idx,vector<vector<int>>&adj,vector<int>&vis,vector<pair<int,int>>&ans,int parity)
{   vis[idx]=1;
    for(int itr:adj[idx])
    {
        if(parity==0 && vis[itr]==0)
        {
            ans.emplace_back(idx,itr);
            vis[itr]=1;
            solve(itr,adj,vis,ans,1);
        }
        if(parity==1 && vis[itr]==0)
        {
            ans.emplace_back(itr,idx);
            vis[itr]=1;
            solve(itr,adj,vis,ans,0);
        }
    }
    return;
}
    ...
    while (t--) 
    {
       ...
       for(int i=0;i<n;i++)
       {
        ...
        {    flag=true;
            ...
            vis[i]=1;
            solve(adj[i][0],adj,vis,ans,0);
            solve(adj[i][1],adj,vis,ans,1);
            break;
