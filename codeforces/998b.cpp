...
int main() {
    ...
    while (t--) {
        ...
        vector<int> order;
        for (int card = 0; card < n * m; ++card) {
            for (int i = 0; i < n; ++i) {
                if (cowCards[i].count(card)) {
        ...
        for (int round = 0; round < m && valid; ++round) {
            for (int cow : order) {
                auto it = cowCards[cow].upper_bound(topCard);
                if (it == cowCards[cow].end()) {
                    valid = false;
                    break;
                }
                topCard = it->first;
                cowCards[cow].erase(it);
            }
        }
