...
bool is_valid(const vector<vector<int>>& positions, int size_n) {
    int value = 1;
    while (value <= size_n) {
        if (positions[value].size() % value != 0)
            return false;
        ++value;
    }
    return true;
...
vector<int> assign_labels(const vector<vector<int>>& positions, int size_n) {
    vector<int> labels(size_n, 0);
    int current_label = 1;
    int value = 1;

    while (value <= size_n) {
        int occurrences = positions[value].size();
        int index = 0;

        while (index < occurrences) {
            int count = 0;
            while (count < value) {
                labels[positions[value][index + count]] = current_label;
                ++count;
            }
            ++current_label;
            index += value;
...
int main() {
    ...
    while(t--)
    {
     ...
     int n;
        ...
        idx = 0;
        while (idx < n) {
            positions[b_array[idx]].pb(idx);
            ++idx;
        }

        if (!is_valid(positions, n)) {
            cout<<-1<<endl;
            continue;
        }
        vector<int> result = assign_labels(positions, n);
