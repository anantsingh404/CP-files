...
int main() {
    ...
    vector<vector<int>> coveredBy(m + 1);
    vector<int> p(n), q(n);
    vector<int> l(n), r(n);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = l[i]; j <= r[i]; ++j) {
            coveredBy[j].push_back(i);
        }
    }

    vector<int> probExist(n), probNotExist(n);

    for (int i = 0; i < n; ++i) {
        int inv = modinv(q[i]);
        probExist[i] = 1LL * p[i] * inv % mod;
        probNotExist[i] = (1 - probExist[i] + mod) % mod;
    }

    int answer = 1;

    for (int i = 1; i <= m; ++i) {
        auto& segs = coveredBy[i];
        int sz = segs.size();
        ...
        for (int j = 0; j < sz; ++j) {
            int idx = segs[j];
            int cur = probExist[idx];
            for (int k = 0; k < sz; ++k) {
                if (k == j) continue;
                cur = 1LL * cur * probNotExist[segs[k]] % mod;
            }
            prob = (prob + cur) % mod;
        }

        answer = 1LL * answer * prob % mod;
    }
