...
//Main Function:
int solve(int i, vector<int>& dp, vector<int>& ans, vector<vector<int>>& temp)
{
    if (i >= dp.size()) {
        return 0;
    }
    ...
    int idx = lower_bound(temp[dp[i]].begin(), temp[dp[i]].end(), i) - temp[dp[i]].begin();
    int m = temp[dp[i]].size();

    if (m - idx >= dp[i]) { // need at least dp[i] occurrences
        int x = temp[dp[i]][idx + dp[i] - 1];
        take = dp[i] + solve(x + 1, dp, ans, temp); // move to next index after x
    }

    nt = solve(i + 1, dp, ans, temp);

    return ans[i] = max(take, nt);
}
...
int main() {
    ...
    while(t--)
    {
     //write your code here
       ...
       {
         temp[dp[i]].push_back(i);
       }
       vector<int>ans(n,-1);
       cout<<solve(0,dp,ans,temp)<<endl;
    }
    return 0;
}
