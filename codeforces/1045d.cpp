...
int main() {
    ...
    int nodes;
        ...
        level.assign(nodes + 1, 0);
        parent.assign(nodes + 1, -1);
        dfs(1, -1);

        int farthestFromRoot = int(max_element(level.begin() + 1, level.end()) - level.begin());

        level.assign(nodes + 1, 0);
        parent.assign(nodes + 1, -1);
        dfs(farthestFromRoot, -1);

        int farthestFromFarthest = int(max_element(level.begin() + 1, level.end()) - level.begin());

        int previous = -1;
        while (farthestFromFarthest != -1 && (int)graph[farthestFromFarthest].size() <= 2) {
            previous = farthestFromFarthest;
            farthestFromFarthest = parent[farthestFromFarthest];
        }

        if (farthestFromFarthest == -1) {
            cout << "-1\n";
            continue;
        }

        int middleNode = farthestFromFarthest;
        int parentNode = parent[middleNode];
        int otherChild = -1;
        for (auto neighbor : graph[middleNode]) {
            if (neighbor != parentNode && neighbor != previous) {
                otherChild = neighbor;
                break;
            }
        }

        cout << parentNode << " " << middleNode << " " << otherChild << "\n";
