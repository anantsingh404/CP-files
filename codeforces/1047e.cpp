...
//Main Function:
long long solve1(const vector<int>& f, int M, int n) {
    long long sum = 0, u = 0;
    int y = 0;
    while (y < M) {
        if (f[y] == 1) { ++u; sum += y; }
    ...
    return sum + 1LL * (n - u) * M;
}

long long solve2(int p, int M, int n, long long k) {
    long long S = 1LL * p * (p - 1) / 2;
    long long rem = n - p;
    if (k % 2 == 0) return S + rem * p;
    return S + rem * (p + 1);
}

long long solve3(int M, int n, long long k) {
    long long S = 1LL * M * (M - 1) / 2;
    long long rem = n - M;
    if (rem == 0) return S;
    if (rem == 1) return S + M;
    if (k % 2 == 0) return S + rem * (M + 1);
    return S + rem * M;
}
int main() {
 ...
 int n;
        ...
        long long ans = (p !=-1)?solve2(p, M, n, k):solve3(M, n, k);
        cout <<ans<<endl;
    }
    return 0;
}
