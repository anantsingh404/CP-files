    ...
    while(t--)
    {
     //write your code here
       ...
       for(int i=1;i<=n;i++)
       {
        ...
        while (i <= n) 
        {
            int revA = ridx(a[i], maxV);
            int prev_ge_a = ob.query(revA);
            ob.update(revA, i);
            int revB = ridx(b[i], maxV);
            int L_ge_b = ob.query(revB);
            long long w =1ll*(n - i + 1);
            S1 += w * 1ll*(L_ge_b);
            if (b[i] < a[i])
            {
                S2 += w * (1ll)*(i - prev_ge_a);
            }
            ++i;
        }
        cout<<(S1-S2)<<endl;
