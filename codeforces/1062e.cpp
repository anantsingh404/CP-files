...
//Main Function:
bool canPlace(vector<ll>& a, ll k, ll x, ll d) {
...
vector<ll> getPositions(vector<ll>& a, ll k, ll d, ll x) {
    vector<ll>pos;
    int n = a.size();
    int i = 0;
    ll mini=0;
    while (i < n) 
    {
        ...
        {
            ll rightPrev = a[i] + d;
            rightPrev=max(rightPrev,mini);
            ll leftNext = a[i + 1] - d;
            while (rightPrev <= leftNext && pos.size() < k) {
                pos.push_back(rightPrev);
                rightPrev++;
                mini=rightPrev;
            }  
        }
        if(i==n-1) {
            ll right = a[i] + d;
            right=max(right,mini);
            while(right<=x && pos.size()<k)
            {
                pos.push_back(right);
                right++;
                    mini=right;
...
int main() {
   ...
   //cout<<ans<<endl;
        vector<ll> res = getPositions(a, k, ans,x);
        for (int i = 0; i < k; i++) {
            cout << res[i] << " ";
        }
        cout << "\n";
