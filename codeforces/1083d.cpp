...
vector<ll> minLen(1001, LLONG_MAX);
ll pairCount(ll x) {
    return x * (x - 1) / 2;
}
int main() {
    ...
    for (int len = 2; pairCount(len) <= limit; ++len)
      {
          vals.push_back({len, (int)pairCount(len)});
      }
    for (int i = 1; i <= limit; i++) 
    {
        for (auto [len, val] : vals) 
        {
            if (val <= i && minLen[i - val] != LLONG_MAX) 
            {
                if (minLen[i - val] + len < minLen[i]) 
                {
                    minLen[i] = minLen[i - val] + len;
                    dp[i] = dp[i - val];
                    dp[i].push_back(len);
                }
            }
