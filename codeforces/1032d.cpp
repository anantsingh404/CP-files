...
int calc_cost(int digit, char a_digit, char b_digit) 
{
    return (digit == (a_digit - '0')) + (digit == (b_digit - '0'));
}

void rsdp(int dp[2][2][2])
 {
    int x = 0;
    while (x < 2) {
        int y = 0;
...
void udp(int pos, string& a, string& b, int dp[2][2][2]) 
{
    int x = 0;
    while (x < 2) {
        int y = 0;
        while (y < 2) {
            if (dp[0][x][y] < INF) {
                int low = x ? 0 : a[pos] - '0';
                int high = y ? 9 : b[pos] - '0';
                
                int d = low;
                while (d <= high) {
                    int nx = x | (d > low);
                    int ny = y | (d < high);
                    int add = calc_cost(d, a[pos], b[pos]);
                    dp[1][nx][ny] = min(dp[1][nx][ny], dp[0][x][y] + add);
                    d++;
                }
            }
            y++;
