...
int main() {
    ...
    while(t--)
    {
     //write your code here
        ...
        vector<long long> covered(n, 0);
        long long l = -1, r = -1;

        for (long long i = 0; i < m; i++) {
            cin >> intervals[i].first >> intervals[i].second;
            covered[intervals[i].first - 1]++;
        ...
        if (l <= r) {
            ...
            if (zeroIndex != -1) {
                ...
                set<long long> startSet, endSet;
                long long index = -1;
                for (long long i = 0; i < m; i++) {
                    startSet.insert(intervals[i].first);
                    endSet.insert(intervals[i].second);
                }
                for (long long i = 1; i <= n; i++) {
                    if (!(startSet.count(i) && endSet.count(i))) {
                        index = i;
                        break;
                    }
                }
                if (index == -1) {
                    ...
                    bool placeLeft = true;
                    if (startSet.count(index) || index == 1)
                        placeLeft = false;
