...
ll computeMaxExpression(ll a, ll b) {
    if (b & 1) {
        if (a & 1) return a * b + 1;
        return -1;
    }

    ll power_of_two = countPowerOfTwo(b);
    if ((a & 1) && power_of_two == 1) return -1;

    ll kmin = (a & 1) ? 2 : 1;
    ll option1 = a * kmin + b / kmin;
    ll option2 = a * (b / 2) + 2;
...
int main() {
    ...
    while(t--)
    {
     //write your code here
        ll a, b;
        cin >> a >> b;
        cout<<computeMaxExpression(a, b)<<endl;
    }
    return 0;
}
