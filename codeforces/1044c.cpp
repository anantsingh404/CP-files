...
int main() {
    ...
    int n;
    cin >> n;
    vector<vector<int>> vec(n + 1);  // vec[i]: portals where query returned i
    vector<int> allPortals(n);
    for (int i = 1; i <= n; i++)
        allPortals[i - 1] = i;
    // Step 1: Group portals by query result
    for (int i = 1; i <= n; i++) {
        int len = query(i, allPortals);
        vec[len].push_back(i);
    }
    ...
    for (int i = n; i >= 1; i--) {
        ...
        for (auto nextPortal : vec[i]) 
        
        {
            if (query(lastPortal, {lastPortal, nextPortal}) == 2) {
                ans.push_back(nextPortal);
