...
static bool cmp(const pair<ll, pair<ll, ll>>& x, const pair<ll, pair<ll, ll>>& y) {
...
int main() {
    ...
    while (t--) {
        ...
        while (v < dp.size()) 
        {
            ...
            while (v < dp.size() && dp[v].first == x)
             {
                p =p+ dp[v].second.first;
                q =q+ dp[v].second.second;
                v=v+1;
            }
            r = p + q;
            t = x;
