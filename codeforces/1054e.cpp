    ...
    while(t--)
    {
     //write your code here
        ...
        for (int i = 0; i < n; i++) {
            ...
            while ((int)mp1.size() > k || i - j + 1 > r) {
                mp1[arr[j]]--;
                if (mp1[arr[j]] == 0) mp1.erase(arr[j]);
                j++;
            }
            while ((int)mp2.size() > p || i - z + 1 > r) {
                mp2[arr[z]]--;
                if (mp2[arr[z]] == 0) mp2.erase(arr[z]);
                z++;
            }
            int left1 = j, left2 = z;
            int min_len_left1 = i - r + 1;
            int min_len_left2 = i - r + 1;
            int max_len_left1 = i - l + 1;
            int max_len_left2 = i - l + 1;
            if (left1 < min_len_left1) left1 = min_len_left1;
            if (left2 < min_len_left2) left2 = min_len_left2;
            if (max_len_left1 < j) continue; 
            int L = max(j, i - r + 1);
            int R = min(z - 1, i - l + 1);
            if (R >= L) count += (R - L + 1);
        }
