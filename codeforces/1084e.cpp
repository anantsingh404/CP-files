...
//Main Function:
const int N = 1e6 + 10;
vector<int> smallest_prime_factor(N);
pair<int,int> solve(int cur)
{
    ...
    while (cur != 1)
    {
        int s = smallest_prime_factor[cur];
...
int main()
{
 ...
 int i = 0;
    ...
    while (i < n)
    {
        ...
        if (smallest_prime_factor[x] != x)
        {
            pair<int,int> res = solve(x);
