...
int arr[4]={-1,1,0,0};
int brr[4]={0,0,-1,1};
void dfs(int i,int j,vector<vector<int>>&vis,int count)
{
   if(count==k)
   {
    ++ans;
   ...
   for(int l=0;l<4;l++)
   {
    int x=i+arr[l];
    int y=j+brr[l];
    if(x>=0 && y>=0 && x<h && y<n && vis[x][y]==-1 && dp[x][y]=='.' )
    {
        dfs(x,y,vis,count+1);
    }
...
int main() {
    ...
    for(int i=0;i<h;i++)
    {
        ...
        {
            if(dp[i][j]=='.')
            {   vector<vector<int>>vis(h,vector<int>(n,-1));
