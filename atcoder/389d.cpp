...
int main() {
    ...
    ll i = 1;
    while (i <= radius - 1) {
        long double temp = (long double)radius * radius - (i + 0.5) * (i + 0.5);
        if (temp < 1.0) {
            i++;
            continue;
        }
        long double s = sqrt(temp);
        ll maxJ = floor(s - 0.5 + 1e-12);
        if (maxJ >= 1) {
            count1 += maxJ;
        }
        i++;
    ...
    long double s2 = sqrt((long double)radius * radius - 0.25);
    ll count2 = floor(s2 - 0.5 + 1e-12);

    ll totalCount = 4 * count1 + 4 * count2 + 1;
    cout << totalCount << '\n';
