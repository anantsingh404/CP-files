...
int main() 
{
    ...
    vector<vector<int>> animal_zoo(M);
    long long min_cost = llm;
    for (int i = 0; i < N; ++i)
     {
        cin >> C[i];
    }
    ...
    for (int mask = 0; mask <59049; ++mask) 
    {
        ...
        {
            int coverage = 0;
            for (int zoo : animal_zoo[animal]) 
            {
                coverage += zoo_counts[zoo];
                if (coverage >= 2) break;
            }
            if (coverage<2) 
            {
        ...
        if (all_covered==true && total_cost<min_cost)
        {
            min_cost=total_cost;
        }
    }
