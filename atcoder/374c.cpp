...
void solve(int idx,vector<long long>&dp,long long sum,set<long long>&ans)
{
    if(idx>=dp.size())
    {
       ans.insert(sum);
    ...
    solve(idx+1,dp,sum+dp[idx],ans);
    solve(idx+1,dp,sum,ans);
}
...
int main() {
    ...
    //long long sum=accumulate(dp.begin(),dp.end(),1ll*0);
    for(int i=0;i<n;i++)
    {
        cin>>dp[i];
    }
    ...
    cout<<*ans.lower_bound(sum/2)<<endl;
