...
int main() {
    ...
    while (numQueries--) {
        ...
        if (queryType == 1) 
        {
            ...
            cin >> newValue;
            if (dq.empty()) {
                dq.push_back({newValue, 0});
            }
             else {
                dq.push_back({newValue, dq.back().second + dq.back().first});
            }
        } 
        else if (queryType == 2)
         {
            ll frontValue = dq.front().first;
            dq.pop_front();
            totalRemoved += frontValue;
        } else if (queryType == 3) {
            ll index;
            cin >> index;
            cout << dq[index - 1].second - totalRemoved << "\n";
        }
    }
