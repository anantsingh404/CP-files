...
double solve(int mask, int x) {
    if (dp[mask][x] != -1) return dp[mask][x];
    double res = 0.0;
    for (int i = 0; i < N; ++i) {
        if (!(mask & (1 << i))) {
            if (x >= C[i]) {
                double success = (P[i] / 100.0) * (S[i] + solve(mask | (1 << i), x - C[i]));
                double failure = ((100 - P[i]) / 100.0) * solve(mask, x - C[i]);
...
int main() {
    ...
    dp.assign(1 << N, vector<double>(X + 1, -1));
    cout << fixed << setprecision(12) << solve(0, X) << endl;
    return 0;
}