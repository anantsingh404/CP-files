...
int main() {
    ...
    for(int i=0;i<n;i++)
    {
        ...
        vp.push_back({vp1[0],vp1[1]});
      }
      float ans=0.0;
      for(int i=0;i<n;i++)
      { int x=vp[i].first.first;
      int y=vp[i].first.second;
      int p=vp[i].second.first;
      int q=vp[i].second.second;
        ans+=1.0*(sqrtf(pow(abs(q-y),2)+pow(abs(x-p),2))/s);
        if(i<n-1)
        {
             x=vp[i+1].first.first;
             y=vp[i+1].first.second;
         ans+=1.0*(sqrtf(pow(abs(q-y),2)+pow(abs(x-p),2))/t);
        }
      }
      cout<<ans<<endl;
