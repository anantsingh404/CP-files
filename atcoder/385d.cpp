...
unordered_map<long long, set<long long>> rowHouses, colHouses;
for (long long i = 0; i < n; i++) {
    long long x, y;
    cin >> x >> y;
    rowHouses[x].insert(y);
    colHouses[y].insert(x);
}

set<pair<long long, long long>> visited;
if (rowHouses[sx].count(sy)) {
...
for (long long i = 0; i < m; i++) {
    ...
    if (d == 'U') {
        auto& row = rowHouses[sx];
        auto it = row.lower_bound(sy);
        while (it != row.end() && *it <= sy + c) {
            visited.insert({sx, *it});
            ++it;
        }
        sy += c;
    } else if (d == 'D') {
        auto& row = rowHouses[sx];
        auto it = row.lower_bound(sy);
        while (it != row.begin() && *(--it) >= sy - c) {
            visited.insert({sx, *it});
        }
        sy -= c;
    } else if (d == 'L') {
        auto& col = colHouses[sy];
        auto it = col.lower_bound(sx);
        while (it != col.begin() && *(--it) >= sx - c) {
            visited.insert({*it, sy});
        }
        sx -= c;
    } else if (d == 'R') {
        auto& col = colHouses[sy];
        auto it = col.upper_bound(sx);
        while (it != col.end() && *it <= sx + c) {
            visited.insert({*it, sy});
            ++it;
        }
        sx += c;
