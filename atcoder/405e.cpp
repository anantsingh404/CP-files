...
int main() {
    ...
    vector<int> A(M), B(M);
    for (int i = 0; i < M; ++i) {
        A[i] = segments[i].first;
        B[i] = segments[i].second;
    }
    ...
    while (Q--) {
        ...
        int crossC = upper_bound(A.begin(), A.end(), C) - A.begin();
        crossC = M - crossC;
        int notStartBeforeC = lower_bound(B.begin(), B.end(), C) - B.begin();
        crossC = crossC - notStartBeforeC;

        int crossD = upper_bound(A.begin(), A.end(), D) - A.begin();
        crossD = M - crossD;
        int notStartBeforeD = lower_bound(B.begin(), B.end(), D) - B.begin();
        crossD = crossD - notStartBeforeD;

        int both = 0;
        for (int i = 0; i < M; ++i) {
            if (segments[i].first < C && segments[i].second > D) {
                both++;
            }
        }

        int result = (crossC + crossD - 2 * both);
        cout << result << '\n';
    }
    return 0;
}