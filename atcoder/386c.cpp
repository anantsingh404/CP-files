...
bool canConvertLinear(const string &S, const string &T, int K) {
    int n = S.size(), m = T.size();

    if (abs(n - m) > K) return false;

    int i = 0, j = 0, mismatchCount = 0;
    ...
    while (i < n && j < m) {
        if (S[i] == T[j]) {
            ...
            mismatchCount++;
            if (mismatchCount > K) return false;

            if (n > m) {
                i++; // Delete a character from S
            } else if (n < m) {
                j++; // Insert a character into S
            } else {
                i++;
                j++; // Replace a character in S
            }
        }
    }

    if (i < n || j < m) mismatchCount++;
