...
int main() {
    ...
    vector<int> lower(n, INT_MAX);
    lower[0] = vp[0].second;
    for (int i = 1; i < n; i++) {
        lower[i] = min(lower[i - 1], vp[i].second);
    }
    ...
    for (int i = 0; i < k; i++) {
        ...
        auto itr = lower_bound(vp.begin(), vp.end(), make_pair(x, 0)) - vp.begin();

        if (itr == 0) {
            if (vp[itr].first > x) {
                ans.push_back(-1);
            } else {
                ans.push_back(vp[itr].second);
            }
        } else if (itr == n) {
            ans.push_back(lower[n - 1]);
        } else {
            if (vp[itr].first > x) {
                --itr;
            }
            ans.push_back(lower[itr]);
        }
    }
