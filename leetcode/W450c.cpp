...
public:
    ...
    int minMoves(vector<string>& mat) {
        int a = mat.size();
        int b = mat[0].size();
        vector<string> flag = mat;
        vector<vector<int>> c(a, vector<int>(b, 0));
        set<char>d;
        priority_queue<tuple<int, int, int>, vector<tuple<int, int, int>>, greater<>> e;
...
int main() {
    ...
    int result = sol.minMoves(mat);
    cout << "Minimum number of moves to reach bottom-right corner: " << result << endl;
