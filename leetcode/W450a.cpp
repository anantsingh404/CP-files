...
public:
    ...
    vector<int> minimumWeight(vector<vector<int>>& e, vector<vector<int>>& q) {
        int a = e.size() + 1;
        vector<vector<int>> c(a, vector<int>(l, -1)); // Ancestors table
        vector<int> d(a, 0); // Depths
        vector<long long> f(a, 0); // Distances from root
        vector<vector<pair<int, int>>> b(a); // Adjacency list
        vector<int> ans;
        ...
        g(0, -1); // DFS to initialize tables from root 0
        ...
        for (int i = 0; i < q.size(); i++) {
            ...
            long long y = dist(u, w);
            long long z = dist(v, w);
...
int main() {
    ...
    // Queries: each query is (u, v, w) — find total distance of triangle (u-v, v-w, w-u) divided by 2
    vector<vector<int>> queries = {
        {2, 4, 0}, // Triangle between 2, 4, 0
        {3, 4, 1}
    };

    vector<int> result = sol.minimumWeight(edges, queries);
