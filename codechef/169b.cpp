...
bool canSortPermutation(vector<int>& arr) {
    unordered_map<int, vector<int>> groups;
    unordered_map<int,vector<int>>mp1;
    int n=arr.size();
    vector<int>tempp(arr.size(),0);


    for (int i=0;i<n;i++){
        int bits = countSetBits(arr[i]);
        groups[bits].push_back(arr[i]);
        mp1[bits].push_back(i);
    ...
    for(int i=0;i<32;i++)
    {
        ...
        vector<int>index=mp1[i];
        sort(temp.begin(),temp.end());
         sort(index.begin(),index.end());
         for(int i=0;i<temp.size();i++)
         {
            tempp[index[i]]=temp[i];
         }
