...
pair<string, int> solve(int bc, int d) 
{
    pair<int, int> st={bc, d};
    ...
    while (kl <= m) 
    {
        ...
        if (kr >= 0 && kr <= m) 
        {
            pair<string,int>lres=solve(bc - 1, kl);
            if (lres.second != -1) 
            {
                pair<string,int>rres=solve(bc-1,kr);
                if (rres.second!=-1)
                 {
                    if(lres.second==0 || rres.second==0) 
                    {
                        string s=lres.first+rres.first;
                        dp[st]={s,0};
                        return dp[st];
                    }
                }
