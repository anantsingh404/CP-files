...
*/
int minSteps(int x1, int y1, int z1, int x2, int y2, int z2, int k) {
    // Calculate the absolute differences in each direction
    int dx = abs(x2 - x1);
    int dy = abs(y2 - y1);
    int dz = abs(z2 - z1);
    
    // Store the differences in a vector and sort in descending order
    vector<int> ans = {dx, dy, dz};
    sort(ans.rbegin(), ans.rend());
    
    // Max difference is ans[0], second max is ans[1], third max is ans[2]

    // If the largest difference can be handled within the sum of the other two differences,
    // we just take the total sum of all differences
    if (ans[0] / k <= (ans[1] + ans[2])) {
        return ans[0] + ans[1] + ans[2];
    }

    // Otherwise, we have to handle the fact that after moving 'K' steps in one direction, 
    // we must switch to another direction.
    int ts=ans[0]/k;
    if(ts%2==0)
    {
        --ts;
...
int main() {
    ...
    while (T--) {
        ...
        cout << minSteps(x1, y1, z1, x2, y2, z2, K) << endl;
    }
