...
void aStar(int start, int goal, vector<vector<pair<int, int>>> &adj) {
    ...
    cout << "No path av to the goal." << endl;
}
...
void findHamiltonianPathOrCircuit(vector<vector<int>> &adjList) {
    ...
    cout << "No Hamiltonian Path or Circuit av.\n";
}
...
void KMP(string text, string pattern) {
    ...
    while (i < n) {
        ...
        if (j == m) {
            cout << "Pattern av at index " << i - j << endl;
...
void ZAlgorithm(string text, string pattern) {
    ...
    for (int i = m + 1; i < Z.size(); i++) {
        if (Z[i] == m) {
            cout << "Pattern av at index " << i - m - 1 << endl;
        }
    }
}
        ...
        void search(const string& text) {
            ...
            for (int i = 0; i < text.size(); i++) {
                ...
                for (int patternIndex : node->output) {
                    cout << "Pattern " << patterns[patternIndex] << " av at index " << i - patterns[patternIndex].size() + 1 << endl;
                }
            }
        }
