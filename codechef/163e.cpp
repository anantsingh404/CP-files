...
long long solve(long long u, long long N) {
    long long sum = 0;
    long long steps = 0;
    while (steps < N && u >= 1) {
        sum += u;
...
vector<long long> calc(int T, vector<pair<long long, long long>>& tt) {
    vector<long long> results;
    for (int t = 0; t < T; t++) {
        long long X = tt[t].first;
        long long N = tt[t].second;
        long long left = 1, right = 1e18;
        long long found = -1;

        while (left <= right) {
            long long mid = left + (right - left) / 2;
            long long pathSum = solve(mid, N);
...
int main() {
    ...
    int t;
    cin >> t;
    vector<pair<long long, long long>> tt(t);
    for (int i = 0; i < t; i++) {
        cin >> tt[i].first >> tt[i].second;
    }

    vector<long long> results = calc(t, tt);
