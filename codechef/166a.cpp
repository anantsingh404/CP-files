...
vector<int> getRanks(vector<int>& arr) {
    int sz = arr.size();
    vector<int> rnk(sz);
    vector<pair<int, int>> pwIdx;

    int i = 0;
    while (i < sz) {
        pwIdx.push_back({arr[i], i});
        i++;
    }

    sort(pwIdx.rbegin(), pwIdx.rend());

    i = 0;
    while (i < sz) {
        rnk[pwIdx[i].second] = i + 1;
        i++;
    }
