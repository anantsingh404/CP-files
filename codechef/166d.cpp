...
int main() {
    ...
    int num = 2;
    while (num <= m) {
        map<int, int> temp = facti(num);
        for (const auto& p : temp) {
            mFact[p.first] += p.second;
        }
        num++;
    }
    ...
    idx = 0;
    while (idx < n) {
        map<int, int> tf = mFact;
        map<int, int> aFactors = facti(arr[idx]);

        for (const auto& p : aFactors) {
            tf[p.first] += p.second;
        }

        long long dc = 1;
        for (const auto& p : tf) {
            dc = (dc * (p.second + 1)) % mod;
        }
