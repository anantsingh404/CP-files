...
int main() {
    ...
    while (t--) 
    {
        ...
        ll best_one = one.empty() ? 0 : one[0];
        ll best_two = two.empty() ? 0 : two[0];
  
        if(two.empty())
        {
            ll attacks = (H + best_one-1)/best_one;
            cout << attacks <<endl;
            continue;
        }
        if(one.empty())
        {
            ll attacks = (H + best_two - 1) / best_two;
            cout << 2 * attacks <<endl;
            continue;
        }
  
        if(2 * best_one >= best_two)
        {
            ll attacks = (H + best_one - 1) / best_one;
            cout << attacks <<endl;
        }
        ...
        {
            ll full_two_attacks = H / best_two;
            ll remaining = H % best_two;
            ll time = 2 * full_two_attacks;
            if(remaining > 0)
            {
                if(remaining <= best_one)
                {
                    time += 1;
                }
                else
