...
*/
int m;
int solve(int idx,string &s,string &rep,vector<vector<int>>&dp,int prev)
{
    if(idx==m)
    {
        return 0;
    ...
    int ans=1e6;
        ...
        if(s[idx]>=x)
        {
            ans=solve(idx+1,s,rep,dp,0);
        }
        if(rep[idx]>=x)
        {
             ans=min(ans,1+solve(idx+1,s,rep,dp,1));
        }
...
rep.push_back(p[x-1]);
     ...
     int x=min(solve(1,s,rep,dp,0),1+solve(1,s,rep,dp,1));
     if(x>=1e6)
     {
        cout<<-1<<endl;
     }
