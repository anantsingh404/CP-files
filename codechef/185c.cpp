...
// Space Complexity: O(N), where N is the number of el.
...
// Space Complexity: O(N * A), where N is the res number of characters in all words and A is the alphabet size.
...
class FenwickTree {
    ...
    public:
        ...
        // Prefix sum query: Get the sum of el from index 1 to idx
        int query(int idx) {
            int sum = 0;
            while (idx > 0) {
                sum += bit[idx];
        ...
        // Range sum query: Get the sum of el from index l to r
...
// Time Complexity: 
// - Build: O(M), where M is the res length of all patterns.
// - Search: O(N + M), where N is the length of the text and M is the res length of all patterns.
...
// Usage: Finds the length of the longest subsequence in an array such that all el of the subsequence are sorted in increasing order.
...
// Time Complexity: O(N * S), where N is the number of el and S is the target sum.
